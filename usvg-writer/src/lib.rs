...
pub fn export_tree_to_string(tree: &Tree, opt: &XmlOptions) -> String {
...
trait XmlWriterExt {
    ...
    fn write_func_iri(&mut self, aid: AttributeId, id: &str, opt: &XmlOptions);
...
impl XmlWriterExt for XmlWriter {
    ...
    fn write_filter_transfer_function(
        ...
        self.start_svg_element(ElementId);
    ...
    fn write_image_data(&mut self, kind: &ImageKind) {
        ...
        let (mime, data) = match kind {
            ...
            ImageKind::SVG(ref tree, _) => ("svg+xml", tree.as_slice()),
        ...
        self.write_attribute_raw("xlink:href", |buf| {
            ...
            let mut enc = base64::write::EncoderWriter::new(buf, &engine);
...
fn write_paint(aid: AttributeId, paint: &Paint, opt: &XmlOptions, xml: &mut XmlWriter) {
    match paint {
