    ...
    for func in svgtypes::FilterValueListParser::from(value) {
        ...
        match func {
            ...
            svgtypes::FilterValue::Contrast(amount) => {
                create_base_filter_func(funcs::convert_contrast(amount), &mut filters, cache)
            }
            svgtypes::FilterValue::Grayscale(amount) => {
                create_base_filter_func(funcs::convert_grayscale(amount), &mut filters, cache)
            }
            svgtypes::FilterValue::HueRotate(angle) => {
                create_base_filter_func(funcs::convert_hue_rotate(angle), &mut filters, cache)
            }
            svgtypes::FilterValue::Invert(amount) => {
                create_base_filter_func(funcs::convert_invert(amount), &mut filters, cache)
            }
            svgtypes::FilterValue::Opacity(amount) => {
                create_base_filter_func(funcs::convert_opacity(amount), &mut filters, cache)
            }
            svgtypes::FilterValue::Sepia(amount) => {
                create_base_filter_func(funcs::convert_sepia(amount), &mut filters, cache)
            }
            svgtypes::FilterValue::Saturate(amount) => {
                create_base_filter_func(funcs::convert_saturate(amount), &mut filters, cache)
            }
...
fn collect_children(
    ...
    for child in filter.children() {
        ...
        let kind = match tag_name {
            ...
            EId::FeImage => image::convert(child, state, cache),
