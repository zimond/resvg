...
pub fn convert(node: svgtree::Node, state: &State) -> Option<tree::SharedPathData> {
    match node.tag_name()? {
...
fn convert_rect(node: svgtree::Node, state: &State) -> Option<tree::SharedPathData> {
...
fn convert_line(node: svgtree::Node, state: &State) -> Option<tree::SharedPathData> {
...
fn points_to_path(node: svgtree::Node, eid: &str) -> Option<tree::PathData> {
...
fn convert_circle(node: svgtree::Node, state: &State) -> Option<tree::SharedPathData> {
...
fn convert_ellipse(node: svgtree::Node, state: &State) -> Option<tree::SharedPathData> {
    let cx = node.convert_user_length(AId::Cx, state, Length::zero());
    let cy = node.convert_user_length(AId::Cy, state, Length::zero());
...
fn ellipse_to_path(cx: f64, cy: f64, rx: f64, ry: f64) -> tree::PathData {
